...
local function phone_page_without_language(page)
	if page.id ~= "accessibility" then
		return page
	end
	local new_page = table.copy(page)
	local content = {}
	for _, entry in ipairs(page.content) do
...
local function phone_build_filtered_pages()
	local phone_pages = {}
	local phone_by_id = {}
	for _, page in ipairs(filtered_pages) do
		local phone_page = phone_page_without_language(page)
		phone_pages[#phone_pages + 1] = phone_page
		phone_by_id[phone_page.id] = phone_page
	end
	return phone_pages, phone_by_id
end
...
function phone_settings_build_state(page_id, query)
	...
	local phone_pages, phone_page_by_id = phone_build_filtered_pages()
	if page_id and page_id ~= "" then
		phone_settings_page_id = page_id
	elseif not phone_settings_page_id or not phone_page_by_id[phone_settings_page_id] then
		phone_settings_page_id = suggested or "accessibility"
	end

	local pages = {}
	local last_section = nil
	for _, other_page in ipairs(phone_pages) do
	...
	local page = phone_page_by_id[phone_settings_page_id]
	return {
		pages = pages,
