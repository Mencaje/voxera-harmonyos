...
function create_contentdb_dlg(type, install_spec)
	-- HarmonyOS: ContentDB download/install is unreliable; use local .zip (content.luanti.org).
	-- Games on other platforms: same local flow when browsing (no online grid).
	if not install_spec and (PLATFORM == "HarmonyOS" or type == "game") then
		return create_local_game_install_dlg()
	end
