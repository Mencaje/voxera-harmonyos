...
local function dir_has_file(path, filename)
...
function pkgmgr.get_folder_type(path)
	if dir_has_file(path, "init.lua") then
		return { type = "mod", path = path }
	end

	if dir_has_file(path, "modpack.conf") or dir_has_file(path, "modpack.txt") then
		return { type = "modpack", path = path }
	end

	if dir_has_file(path, "game.conf") then
		return { type = "game", path = path }
	end

	if dir_has_file(path, "texture_pack.conf") then
...
local function game_install_basename(game_folder)
	local conf = Settings(game_folder .. DIR_DELIM .. "game.conf")
	if conf then
		local name = conf:get("name")
		if name and name ~= "" then
			return pkgmgr.normalize_game_id(name)
		end
	end
	local folder = get_last_folder(cleanup_path(game_folder))
	if folder and folder ~= "" then
		return pkgmgr.normalize_game_id(folder)
	end
	return nil
end
...
function pkgmgr.install_dir(expected_type, path, basename, targetpath)
	...
	if not targetpath then
		...
		elseif basefolder.type == "game" then
			...
			if not basename then
				basename = game_install_basename(basefolder.path)
			end
		else
