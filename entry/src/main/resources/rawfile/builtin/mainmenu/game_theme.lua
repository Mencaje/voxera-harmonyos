...
local function try_set_background_from_basename(identifier, basename)
	for _, extension in pairs(valid_image_extensions) do
		local path = defaulttexturedir .. DIR_DELIM .. basename .. extension
...
function mm_game_theme.set_engine_single(identifier)
	...
	-- Voxera branding (replaces upstream menu_header.png / Luanti wordmark)
	if identifier == "header" then
		if try_set_background_from_basename(identifier, "voxera_menu_header") then
			return true
		end
	end
