...
--------------------------------------------------------------------------------
local function fetch_public_serverlist()
	...
	if not response.succeeded then
		local hint = response.timeout and "timeout" or ("HTTP " .. tostring(response.code or 0))
		core.log("warning", "Public server list fetch failed: " .. hint .. " url=" .. url)
		return { ok = false, err = hint }
	end

	local retval = core.parse_json(response.data)
	if type(retval) ~= "table" or type(retval.list) ~= "table" then
		core.log("warning", "Public server list: invalid JSON from " .. url)
		return { ok = false, err = "invalid_json" }
	end
	return { ok = true, list = retval.list }
end
...
function serverlistmgr.sync()
	...
	core.handle_async(
		function(param)
			return fetch_public_serverlist()
		end,
		nil,
		function(result)
			public_downloading = false
			if type(result) ~= "table" or not result.ok then
				local fetch_err = (type(result) == "table" and result.err) or "unknown"
				serverlistmgr.servers = {{
					name = fgettext("Could not load server list"),
					description = fgettext("Network error (@1). Check Internet permission and connection, then tap refresh.", fetch_err)
				}}
				core.event_handler("Refresh")
				return
			end
			local favs = order_server_list(result.list)
			if favs[1] then
				serverlistmgr.servers = favs
			else
				serverlistmgr.servers = {{
					name = fgettext("No servers found"),
					description = fgettext_ne(
						"Try reenabling public serverlist and check your internet connection.")
				}}
			end
			core.event_handler("Refresh")
		end
