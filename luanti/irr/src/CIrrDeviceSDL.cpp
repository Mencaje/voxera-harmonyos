...
#ifdef __OHOS__
/* Do not include native_interface_xcomponent.h here — it clashes with irrlicht Keycodes.h */
...
void CIrrDeviceSDL::resetReceiveTextInputEvents()
{
	...
	if (elem && elem->isVisible() && elem->acceptsIME()) {
		...
		IsInBackground(false)
{
	if (++SDLDeviceInstances == 1) {
...
#if defined(_IRR_COMPILE_WITH_JOYSTICK_EVENTS_) && !defined(__OHOS__)
		/* OHOS SDL build has no joystick driver in SDL_joystick_drivers[] — Init fails. */
...
#endif
		{
			...
			os::Printer::log("Unable to initialize SDL", err, ELL_ERROR);
#ifdef __OHOS__
			{
				char buf[512];
				const char *msg = (err && err[0]) ? err : "SDL_Init failed (no SDL_GetError)";
				snprintf(buf, sizeof(buf), "SDL_Init: %s", msg);
				porting_ohosEngineStatusSet(buf);
			}
#endif
			Close = true;
		}
...
bool CIrrDeviceSDL::createWindowWithContext()
{
...
#ifdef __OHOS__
		/* 2in1 / XComponent: EglPreview uses ES3; ES2 context often fails here. */
		SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 3);
...
#ifdef __OHOS__
	{
		unsigned sw = 0;
		unsigned sh = 0;
		if ((Width == 0 || Height == 0) && porting_ohosGetSurfaceSize(&sw, &sh)) {
			Width = sw;
			Height = sh;
			ScaleX = 1.0f;
			ScaleY = 1.0f;
		...
		if (Width > 0 && Height > 0) {
			...
			porting_ohosEngineStatusSet("OHOS: test frame (sky blue) swapped");
		} else {
			porting_ohosEngineStatusSet("OHOS: surface size is 0");
		}
	}
#endif
...
//! create the driver
void CIrrDeviceSDL::ensureGLContextCurrent()
{
#ifdef __OHOS__
	if (Window && Context) {
		SDL_GL_MakeCurrent(Window, Context);
...
#ifdef __OHOS__
		porting_ohosEngineStatusSet("OHOS: createOGLES2Driver failed");
#endif
	}
}
...
bool CIrrDeviceSDL::run()
{
	...
	while (!Close && wrap_PollEvent(&SDL_event)) {
...
#ifndef __OHOS__
			if (SDL_TextInputActive(Window) && GUIEnvironment) {
				gui::IGUIElement *imeElem = GUIEnvironment->getFocus();
				if (imeElem && imeElem->isVisible() && imeElem->acceptsIME() &&
						!keyIsKnownSpecial(key) && (keymod & SDL_KMOD_CTRL) == 0)
					break;
			}
#endif
		...
		case SDL_EVENT_QUIT:
#ifdef __OHOS__
			porting_ohosEngineStatusSet("SDL quit (window close)");
#endif
			Close = true;
			break;
...
{
#ifdef __OHOS__
	/* ArkUI owns the window title; SDL OHOS path calls NAPI from the game
	 * thread and can crash (null env / invalid tsfn). Menu loop updates caption every frame. */
	(void)text;
	return;
#endif
	core::stringc textc;
...
{
#ifdef __OHOS__
	return porting::ohosIsFullscreen();
#else
	if (!Window)
		return false;
...
{
#ifdef __OHOS__
	/* HarmonyOS: use native immersive fullscreen (hide system bars), not SDL window flags. */
	porting::ohosNotifyFullscreen(fullscreen);
	return true;
#else
	if (!Window)
		return false;
...
#ifdef __OHOS__
	/* ArkUI injects keys via NAPI; SDL_WINDOW_INPUT_FOCUS is often unset on foreign XComponent. */
	return (windowFlags & SDL_WINDOW_SHOWN) != 0;
#endif
#ifdef _IRR_USE_SDL3_
