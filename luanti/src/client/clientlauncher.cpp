...
#ifdef __OHOS__
	extern void porting_ohosEngineStatusSet(const char *);
	porting_ohosEngineStatusSet("init_engine: start");
	porting::ohosApplyClientDefaults();
#endif
	receiver = new MyEventReceiver();
	// Note: this can throw
	m_rendering_engine = new RenderingEngine(receiver);
#ifdef __OHOS__
	porting_ohosEngineStatusSet("init_engine: done");
#endif
}
...
static video::ITexture *loadTexture(video::IVideoDriver *driver, const char *path)
{
	...
	if (auto *image = RenderingEngine::loadImageFromDisk(driver, path); image) {
