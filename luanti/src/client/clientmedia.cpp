...
{
	if (!m_cache_scan_done) {
		scanCacheBatch(client);
		if (!m_cache_scan_done)
			return;
...
namespace {
constexpr size_t MEDIA_CACHE_SCAN_BATCH_FILES = 64;
constexpr u64 MEDIA_CACHE_SCAN_SLICE_MS = 33;
}

void ClientMediaDownloader::scanCacheBatch(Client *client)
{
	if (m_cache_scan_done)
		return;

	if (!m_cache_scan_started) {
		m_cache_scan_started = true;
		m_uncached_count = m_files.size();
		m_cache_scan_it = m_files.begin();
	}

	std::wstring loading_text = wstrgettext("Media...");
	u64 slice_start = porting::getTimeMs();
	size_t processed = 0;

	while (m_cache_scan_it != m_files.end()) {
		const std::string &name = m_cache_scan_it->first;
		FileStatus *filestatus = m_cache_scan_it->second;
		...
		if (processed >= MEDIA_CACHE_SCAN_BATCH_FILES ||
				porting::getDeltaMs(slice_start, porting::getTimeMs()) >=
						MEDIA_CACHE_SCAN_SLICE_MS) {
			client->drawLoadScreen(loading_text,
					MEDIA_CACHE_SCAN_SLICE_MS / 1000.0f, 30);
#ifdef __OHOS__
			{
				const s32 done = (s32)m_files.size() - m_uncached_count;
				char buf[96];
				snprintf(buf, sizeof(buf), "Media cache %d/%zu",
						done, m_files.size());
				porting::ohosLogMediaProgress(buf);
			}
#endif
			return;
		}
	...
	m_cache_scan_done = true;
	assert(m_uncached_received_count == 0);
	finishInitialStepAfterCache(client);
}

void ClientMediaDownloader::finishInitialStepAfterCache(Client *client)
{
	...
	if (!USE_CURL || m_uncached_count == 0 || m_remotes.empty()) {
		...
		{
			char buf[96];
			snprintf(buf, sizeof(buf), "Media from server: %d files",
					m_uncached_count);
			porting::ohosLogMediaProgress(buf);
		}
#endif
...
#ifdef __OHOS__
		porting::ohosLogMediaProgress("Media: remote CDN index...");
#endif
	}
}
...
void ClientMediaDownloader::startConventionalTransfers(Client *client)
{
	...
	if (m_uncached_received_count != m_uncached_count) {
		char buf[96];
		snprintf(buf, sizeof(buf), "Media request %d files",
				m_uncached_count - m_uncached_received_count);
		porting::ohosLogMediaProgress(buf);
	}
#endif
...
#ifdef __OHOS__
	if ((m_uncached_received_count % 200) == 0 ||
			m_uncached_received_count == m_uncached_count) {
		char buf[96];
		snprintf(buf, sizeof(buf), "Media recv %d/%d",
				m_uncached_received_count, m_uncached_count);
		porting::ohosLogMediaProgress(buf);
	}
#endif
