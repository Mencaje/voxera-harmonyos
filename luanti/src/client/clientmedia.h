...
public:
	...
	float getProgress() const {
		if (m_files.empty())
			return 0.0f;
		/* While scanning the on-disk cache, uncached_count shrinks per hit. */
		if (!m_cache_scan_done) {
			return 1.0f * (m_files.size() - m_uncached_count) /
				(float)m_files.size();
		}
...
private:
	...
	void scanCacheBatch(Client *client);
	void finishInitialStepAfterCache(Client *client);
	...
	bool m_cache_scan_started = false;
	bool m_cache_scan_done = false;
	std::map<std::string, FileStatus *>::iterator m_cache_scan_it;
