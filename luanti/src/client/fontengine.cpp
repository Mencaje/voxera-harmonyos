...
void FontEngine::setMediaFont(const std::string &name, const std::string &data)
{
	...
	if (!face) {
		warningstream << "Ignoring invalid media font: " << name << std::endl;
		return;
	}
	m_media_faces.emplace(name, std::move(face));
	refresh();
}
...
#ifdef __OHOS__
	/* FreeType FT_New_Face(path) can fail on OHOS sandbox; load via ReadFile. */
	if (!face) {
		std::string data;
		if (fs::ReadFile(filename, data, false))
			face = irr_ptr<gui::SGUITTFace>(gui::SGUITTFace::createFace(std::move(data)));
	}
#endif
	if (!face)
		return nullptr;
...
gui::IGUIFont *FontEngine::initFont(FontSpec spec)
{
...
#ifdef __OHOS__
	/* Server media fonts are unreliable on OHOS; always use bundled TTFs. */
	spec.allow_server_media = false;
#endif
	...
	u32 size = rangelim(spec.size * RenderingEngine::getFontDisplayDensity() *
	...
	auto createFont = [&](gui::SGUITTFace *face) -> gui::CGUITTFont* {
		...
		auto *font = gui::CGUITTFont::createTTFont(m_env,
				face, size, antialias, spec.mode != _FM_Fallback, font_shadow,
				font_shadow_alpha);
	...
	if (spec.allow_server_media) {
		...
		if (it != m_media_faces.end() && it->second) {
	...
	try_paths.push_back(g_settings->get(path_setting));
	try_paths.push_back(Settings::getLayer(SL_DEFAULTS)->get(path_setting));
#ifdef __OHOS__
	const std::string ohos_droid = porting::getDataPath("fonts" DIR_DELIM "DroidSansFallbackFull.ttf");
	const std::string ohos_arimo = porting::getDataPath("fonts" DIR_DELIM "Arimo-Regular.ttf");
	const std::string ohos_cousine = porting::getDataPath("fonts" DIR_DELIM "Cousine-Regular.ttf");
	if (spec.mode == FM_Mono) {
		try_paths.push_back(ohos_cousine);
		try_paths.push_back(ohos_droid);
		try_paths.push_back(ohos_arimo);
	} else if (spec.mode == _FM_Fallback) {
		try_paths.push_back(ohos_arimo);
		try_paths.push_back(ohos_droid);
	} else {
...
#endif
	for (const std::string &font_path : try_paths) {
		if (font_path.empty())
			continue;
		...
		if (auto *face = getOrLoadFace(font_path)) {
			if (auto *font = createFont(face))
				return font;
			errorstream << "FontEngine: Face loaded but CGUITTFont failed for '"
					<< font_path << "'" << std::endl;
		} else {
		...
		if (fs::PathExists(font_path)) {
			char buf[384];
			snprintf(buf, sizeof(buf), "字体存在但加载失败: %s", font_path.c_str());
