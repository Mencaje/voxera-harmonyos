...
#ifdef __OHOS__
	porting::ohosEngineStatusSet("in_game: world");
#endif
...
#ifdef __OHOS__
		warningstream << "IPv6 listen disabled on OHOS; binding IPv4 instead of "
				<< bind_addr.serializeString() << std::endl;
		bind_addr = Address(0, 0, 0, 0, port);
		if (!bind_str.empty()) {
			try {
				bind_addr.Resolve(bind_str.c_str());
			} catch (const ResolveError &e) {
				warningstream << "Resolving bind address \"" << bind_str
						<< "\" failed: " << e.what()
						<< " -- Listening on all IPv4 addresses." << std::endl;
			}
		}
#else
...
{
#ifdef __OHOS__
	porting::ohosApplyClientLoadingDefaults();
	porting::ohosLogMediaProgress("Loading server content...");
#endif
	input->clear();
