...
#ifdef __OHOS__
void MyEventReceiver::syncFromSdlKeyboard()
{
	const Uint8 *state = SDL_GetKeyboardState(nullptr);

	for (const auto &entry : keysListenedFor) {
		const KeyPress &keyCode = entry.first;
		if (keyCode.getType() != KeyPress::InputType::KEYBOARD)
			continue;
		const u32 scancode = keyCode.getScancode();
		if (scancode <= SDL_SCANCODE_UNKNOWN || scancode >= SDL_NUM_SCANCODES)
			continue;
		bool down = porting::ohosIsScancodePressed(scancode);
		if (!down && state)
			down = state[scancode] != 0;
		setKeyDown(keyCode, down);
	}
}
#endif
