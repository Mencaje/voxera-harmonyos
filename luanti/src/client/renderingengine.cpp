...
static IrrlichtDevice *createDevice(SIrrlichtCreationParameters params, std::optional<video::E_DRIVER_TYPE> requested_driver)
{
...
#ifdef __OHOS__
	{
		...
		if (phase && phase[0] && strcmp(phase, "engine idle") != 0)
			throw BaseException(phase);
		if (sdlerr && sdlerr[0])
			throw BaseException(sdlerr);
	}
#endif
...
RenderingEngine::RenderingEngine(MyEventReceiver *receiver)
{
	...
	unsigned sw = 0, sh = 0;
	if (porting::ohosGetSurfaceSize(&sw, &sh)) {
		screen_w = sw > 0 ? static_cast<u16>(sw) : 1280;
		screen_h = sh > 0 ? static_cast<u16>(sh) : 720;
	} else {
		screen_w = 1280;
		screen_h = 720;
	}
	...
	if (driver) {
		...
		std::string msg = std::string("video driver: ") +
			getVideoDriverName(driver->getDriverType());
		porting_ohosEngineStatusSet(msg.c_str());
	}
#endif
...
{
	irr_ptr<video::IImage> img(loadImageFromDisk(driver,
			porting::path_share + "/textures/base/pack/logo.png"));
	if (!img) {
		...
		int percent, float *indef_pos)
{
	...
	if (g_menuclouds && g_menucloudsmgr && g_settings->getBool("menu_clouds")) {
...
video::IImage *RenderingEngine::loadImageFromDisk(video::IVideoDriver *driver,
		const std::string &path)
{
	if (!driver || path.empty() || !fs::PathExists(path))
		return nullptr;

	if (video::IImage *image = driver->createImageFromFile(path.c_str()))
		return image;

	std::string data;
	if (!fs::ReadFile(path, data, false) || data.empty())
		return nullptr;

	IrrlichtDevice *device = get_raw_device();
	if (!device)
		return nullptr;

	io::IReadFile *memfile = device->getFileSystem()->createMemoryReadFile(
			data.data(), data.size(), fs::GetFilenameFromPath(path.c_str()));
	if (!memfile)
		return nullptr;

	video::IImage *image = driver->createImageFromFile(memfile);
	memfile->drop();
	return image;
}
...
float RenderingEngine::getDisplayDensity()
{
	...
	if (dpi == 0.0f)
		dpi = 96.0f;
	float scale = dpi / 96.0f * user_factor;
	return rangelim(scale, 0.75f, 1.35f);
#else
	float dpi = get_raw_device()->getDisplayDensity();
	if (dpi == 0.0f)
		dpi = 96.0f;
...
float RenderingEngine::getFontDisplayDensity()
{
#if defined(__OHOS__)
	float base = getDisplayDensity();
	const v2u32 sz = getWindowSize();
	const s32 actual_min = (s32)std::min(sz.X, sz.Y);
	const s32 min_dim = ohosFormspecMinScreenDim(actual_min);
	float boost = (float)min_dim / 900.0f;
	/* Fullscreen: extra font size on top of dialog scale. */
	if (actual_min >= 1200)
		boost *= 1.42f;
	if (actual_min >= 1600)
		boost *= 1.15f;
	return base * rangelim(boost, 1.0f, 1.92f);
#else
	return getDisplayDensity();
#endif
}

float RenderingEngine::getHudDisplayDensity()
{
#if defined(__OHOS__)
	float base = getDisplayDensity();
	const v2u32 sz = getWindowSize();
	const s32 actual_min = (s32)std::min(sz.X, sz.Y);
	float boost = 1.0f;
	/* Window expanded (maximize / large presentation): nudge HUD cluster up a bit. */
	if (actual_min > 1000)
		boost += (float)(actual_min - 1000) / 4000.0f;
	auto *device = get_raw_device();
	if (device && device->isWindowMaximized())
		boost *= 1.10f;
	return base * rangelim(boost, 1.0f, 1.32f);
#else
	return getDisplayDensity();
#endif
}
...
#if defined(__OHOS__)
s32 RenderingEngine::ohosFormspecMinScreenDim(s32 actual_min_dim)
{
	/*
	...
	return rangelim(eff, floor_dim, ceil_dim);
}

v2u32 RenderingEngine::ohosFormspecLayoutSize(v2u32 actual)
{
	v2u32 out;
	out.X = (u32)rangelim((s32)(actual.X * 0.98f + 0.5f), 960, 3000);
	out.Y = (u32)rangelim((s32)(actual.Y * 0.98f + 0.5f), 540, 1900);
	return out;
}
#endif
