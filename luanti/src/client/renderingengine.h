...
public:
	...
	/** OHOS: font pixel size only; may exceed layout density on large windows. */
	static float getFontDisplayDensity();
	/** OHOS: in-game hotbar / statbar scale; grows slightly on large or maximized windows. */
	static float getHudDisplayDensity();
#if defined(__OHOS__)
	/** Formspec/dialog scale follows window size (ratio + floor/ceil clamps). */
	static s32 ohosFormspecMinScreenDim(s32 actual_min_dim);
	static v2u32 ohosFormspecLayoutSize(v2u32 actual);
#endif
	...
	/** Load PNG/etc. from disk; on OHOS falls back to fs::ReadFile + memory loader. */
	static video::IImage *loadImageFromDisk(video::IVideoDriver *driver,
