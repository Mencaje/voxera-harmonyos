...
#ifdef __OHOS__
			{
				std::string rel = std::string("client") + DIR_DELIM + "shaders" + DIR_DELIM
						+ name_of_shader + DIR_DELIM + filename;
				std::string fallback = porting::path_share + DIR_DELIM + rel;
				if (fs::PathExists(fallback))
					path = fallback;
			}
			/* Geometry shaders are optional (cloud_shader has no opengl_geometry.glsl). */
			if (path.empty() && filename != "opengl_geometry.glsl") {
				extern void porting_ohosEngineStatusSet(const char *);
				char buf[512];
				snprintf(buf, sizeof(buf), "shader missing: %s (share=%s)",
						combined.c_str(), porting::path_share.c_str());
