...
video::ITexture *MenuTextureSource::getTexture(const std::string &name, u32 *id)
{
	...
	video::IImage *image = RenderingEngine::loadImageFromDisk(m_driver, name);
	if (!image)
		return NULL;
	...
	m_kill(kill)
{
	...
	const auto &report_fatal_error = [&] (const std::string &detail) {
		...
		const char *generic = gettext("Failed to load main menu script!");
		/* Copy detail before writing errormessage — detail may point at errormessage.c_str(). */
		if (!detail.empty() &&
				(detail.find("missing file:") != std::string::npos ||
				 detail.find("Failed to load and run script") != std::string::npos)) {
			m_data->script_data.errormessage = detail;
		} else if (!detail.empty() && detail != generic &&
				!str_starts_with(detail, generic)) {
			m_data->script_data.errormessage = generic;
		...
		/* Do not set m_kill here — that poisons ClientLauncher and exits the app. */
		RenderingEngine::showErrorMessageBox(m_data->script_data.errormessage.c_str());
#ifdef __OHOS__
		porting::ohosEngineStatusSet(m_data->script_data.errormessage.c_str());
#endif
	};
	...
	try {
		...
		if (!loadMainMenuScript()) {
			report_fatal_error(m_data->script_data.errormessage);
			return;
		}

#ifdef __OHOS__
		porting::ohosEngineStatusSet("main_menu: script ok");
#endif
		run();
	} catch (ModError &e) {
...
#ifdef __OHOS__
		porting::ohosEngineStatusSet(e.what());
#endif
	}
}
...
bool GUIEngine::loadMainMenuScript()
{
	...
	if (!fs::PathExists(script)) {
		m_data->script_data.errormessage = "missing file: " + script +
				" (share=" + porting::path_share + ")";
#ifdef __OHOS__
		errorstream << m_data->script_data.errormessage << std::endl;
#endif
		return false;
	}
	const std::string mainmenu_init = m_scriptdir + DIR_DELIM + "init.lua";
	if (!fs::PathExists(mainmenu_init)) {
		m_data->script_data.errormessage = "missing file: " + mainmenu_init +
				" (share=" + porting::path_share + ")";
#ifdef __OHOS__
		errorstream << m_data->script_data.errormessage << std::endl;
#endif
...
#ifdef __OHOS__
		{
			...
			if (porting::ohosTakePickResult(pick_form, pick_path, pick_ok) && m_buttonhandler) {
				if (pick_ok) {
					...
					porting::ohosEngineStatusSet(buf);
				} else {
					porting::ohosEngineStatusSet("pick canceled");
				}
				StringMap fields;
				if (pick_ok)
				...
				m_script->handleMainMenuButtons(fields);
			}
		}
#endif
...
{
	/* OHOS does not create g_menuclouds (custom cloud shaders); Lua may still enable menu_clouds. */
	if (!g_menuclouds || !g_menucloudsmgr) {
		drawBackground(m_rendering_engine->get_video_driver());
		return;
	}
...
#ifdef __OHOS__
		/* Touch/fullscreen formspecs often start at y=0; reserve top band for menu_header. */
		const s32 band_h = (s32)screensize.Height / 4;
		if (band_h >= 16) {
			max_rect = core::rect<s32>(4, 4, screensize.Width - 8, band_h);
		} else
#endif
			return;
	}
