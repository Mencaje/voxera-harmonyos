	...
	result(request_)
{
	...
	bool verify_cert = g_settings->getBool("curl_verify_cert");
	curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, verify_cert ? 1L : 0L);
	curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, verify_cert ? 2L : 0L);
}
	...
	if (!request.quiet) {
		if (res != CURLE_OK) {
			...
			porting::ohosEngineStatusSet(
				(std::string("HTTP: ") + curl_easy_strerror(res)).c_str());
#endif
...
#ifdef __OHOS__
	if (!g_httpfetch_thread->start()) {
		errorstream << "httpfetch_init: failed to start CurlFetchThread on OHOS"
				<< " (sync menu HTTP uses curl_easy_perform instead)" << std::endl;
	}
#endif
}
...
/** Blocking easy-perform (menu async jobs). Avoids curl_multi on HarmonyOS. */
static bool httpfetch_sync_direct(const HTTPFetchRequest &fetch_request,
		HTTPFetchResult &fetch_result)
{
	CurlHandlePool pool;
	HTTPFetchOngoing ongoing(fetch_request, &pool);
	const CURLcode res = ongoing.start(nullptr);
	const HTTPFetchResult *completed = ongoing.complete(res);
	fetch_result = completed ? *completed : HTTPFetchResult(fetch_request);
	return true;
}
		...
		HTTPFetchResult &fetch_result, long interval)
{
	if (const Thread *thread = Thread::getCurrentThread()) {
		...
		return httpfetch_sync_direct(fetch_request, fetch_result);
#else
