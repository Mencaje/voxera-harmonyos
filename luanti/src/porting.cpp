...
static std::string detectSystemInfo()
{
	...
	struct utsname osinfo;
	uname(&osinfo);
	oss << "HarmonyOS " << osinfo.release << " " << osinfo.machine;
	return oss.str();
#else /* POSIX */
	struct utsname osinfo;
	uname(&osinfo);
...
extern bool setSystemPaths(); // defined in porting_ohos.cpp
...
static bool open_uri(const std::string &uri)
{
	...
	return true;
#elif defined(__OHOS__)
	porting::ohosRequestOpenUrl(uri);
	return true;
#elif defined(__APPLE__)
...
#if defined(__OHOS__)
	std::string open_path = fs::AbsolutePath(path);
	const std::string &pub = porting::ohosGetPublicUserDir();
	if (!pub.empty() && fs::PathStartsWith(open_path, porting::path_user))
		open_path = pub;
	porting::ohosRequestOpenLocalPath(open_path);
	return true;
#else
