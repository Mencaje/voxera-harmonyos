...
#if defined(__OHOS__)
	if (value && (!g_menuclouds || !g_menucloudsmgr))
		warningstream << "Main menu 3D clouds unavailable on HarmonyOS; using background."
				<< std::endl;
#endif

	engine->m_clouds_enabled = value && g_menuclouds && g_menucloudsmgr;
...
int ModApiMainMenu::l_copy_dir(lua_State *L)
{
	...
	if (!retval) {
		char buf[512];
		snprintf(buf, sizeof(buf), "copy_dir failed: %s -> %s", source, destination);
...
int ModApiMainMenu::l_extract_zip(lua_State *L)
{
	...
	if (!ok) {
		char buf[512];
		snprintf(buf, sizeof(buf), "extract_zip failed: %s", zipfile);
		porting::ohosEngineStatusSet(buf);
	}
#endif
...
#if defined(__OHOS__)
/******************************************************************************/
int ModApiMainMenu::l_ohos_set_status(lua_State *L)
{
...
int ModApiMainMenu::l_ohos_set_zip_drop_target(lua_State *L)
{
	const char *formname = luaL_optstring(L, 1, "");
...
int ModApiMainMenu::l_show_path_select_dialog(lua_State *L)
{
	...
	(void)title;
	if (is_file_select) {
		porting::ohosRequestPickZipFile(formname);
		return 0;
	}
#endif
