...
{
    if (!title || !g_napiCallback || !g_napiCallback->tsfn) {
        return;
    }
    cJSON *root = cJSON_CreateObject();
    if (root == NULL) {
...
{
    if (!g_napiCallback || !g_napiCallback->tsfn) {
        SDL_Log("OHOS_NAPI_SetWindowStyle: NAPI not ready (embedded SDL), ignored");
        return;
    }
    cJSON *root = cJSON_CreateObject();
    if (root == NULL) {
...
{
#ifdef VOXERA_EMBEDDED_SDL
    (void)xcomponent;
    (void)hotX;
    (void)hotY;
    /* Games use XComponent input + ArkUI pointer APIs; no OHOS custom cursor NAPI. */
    return 1;
#endif
    if (!xcomponent || !xcomponent->format || !xcomponent->pixels) {
        SDL_SetError("OHOS_CreateCustomCursor: invalid surface");
        return -1;
    }
    if (!OHOS_NapiBridgeReady()) {
        return 1;
    }
    cJSON *root = cJSON_CreateObject();
    if (root == NULL) {
...
extern SDL_bool OHOS_NapiBridgeReady(void);
...
static SDL_atomic_t g_ohosRelativeMouseEnabled;
static OHOS_VoxeraRelativeMouseNotifier g_relativeMouseNotifier;

extern "C" void OHOS_VoxeraSetRelativeMouseNotifierPlugin(OHOS_VoxeraRelativeMouseNotifier notifier)
{
    g_relativeMouseNotifier = notifier;
}

extern "C" SDL_bool OHOS_GetRelativeMouseEnabled(void)
{
    return SDL_AtomicGet(&g_ohosRelativeMouseEnabled) ? SDL_TRUE : SDL_FALSE;
}

extern "C" SDL_bool OHOS_SupportsRelativeMouse(void)
{
    return SDL_TRUE;
}
...
extern "C" SDL_bool OHOS_SetRelativeMouseEnabled(SDL_bool enabled)
{
    SDL_AtomicSet(&g_ohosRelativeMouseEnabled, enabled ? 1 : 0);
    OHOS_VoxeraResetRelativeMouseTracking();
    if (g_relativeMouseNotifier) {
        g_relativeMouseNotifier(enabled ? 1 : 0);
    }
    return SDL_TRUE;
}
...
napi_value SDLNapi::OHOS_OnNativeFocusChanged(napi_env env, napi_callback_info info)
{
    ...
    SDL_SendWindowEvent(curWindow,
        focus ? SDL_WINDOWEVENT_FOCUS_GAINED : SDL_WINDOWEVENT_FOCUS_LOST, 0, 0);
    return nullptr;
}
