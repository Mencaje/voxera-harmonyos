...
static SDL_Cursor *OHOS_CreateEmptyCursor()
{
    if (!empty_cursor) {
#ifdef VOXERA_EMBEDDED_SDL
        empty_cursor = OHOS_WrapCursor(1, 0);
#else
...
static void OHOS_WarpMouse(SDL_Window *window, int x, int y)
{
    SDL_Mouse *mouse = SDL_GetMouse();
    if (!window || !mouse) {
        return;
    }
    /* No OS warp on XComponent; keep SDL state in sync for Luanti center-lock. */
    mouse->x = x;
    mouse->y = y;
    mouse->last_x = x;
    mouse->last_y = y;
...
void OHOS_DeliverRelativeMouseMotion(SDL_Window *window, int dx, int dy)
{
    ...
    windowsize.action = ACTION_MOVE;
    windowsize.state = 0;
    windowsize.x = (float)dx;
    windowsize.y = (float)dy;
    OHOS_OnMouse(window, &windowsize, SDL_TRUE);
}
...
void OHOS_OnMouse(SDL_Window *window, OHOSWindowSize *windowsize, SDL_bool relative)
{
    ...
    switch (windowsize->action) {
        case ACTION_PRESS:
            ...
            if (!relative && !OHOS_GetRelativeMouseEnabled()) {
                SDL_SendMouseMotion(window, 0, SDL_FALSE, (int)windowsize->x, (int)windowsize->y);
            }
            if (button) {
        ...
        case ACTION_RELEASE:
            /* XComponent passes the released button in state, not a cleared mask. */
            button = TranslateButton(windowsize->state);
            last_state &= ~windowsize->state;
            if (!relative && !OHOS_GetRelativeMouseEnabled()) {
                SDL_SendMouseMotion(window, 0, SDL_FALSE, (int)windowsize->x, (int)windowsize->y);
